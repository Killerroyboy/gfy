...
- [ ] **Step 3: Implement.** START HERE additions follow sheet-polish.gs's existing content-anchored row pattern (read :100-160 first): one block "Captain scoring links" (one row per `rosterTeams_` value: team label + `https://killerroyboy.github.io/gfy/#score?team=<encodeURIComponent(team)>`), one block "Form Team dropdown — paste exactly this list", one note row "Team names freeze once links go out; a rename must also be applied to Scores."
  **CONFIG-HOME MIGRATION (added after Task-4 review — closes the form_url population gap):** the existing START HERE "Scoring form URL (paste once)" cell (sheet-polish.gs ~:123-148) is RETIRED: its label text changes to "Scoring config moved → Info tab" and the polish comment + README point at TWO Info keys instead: `score_endpoint` (the /exec URL) and `form_url` (the raw form link — the fallback the scorer renders; key name pinned by Task 4, index.html scFormUrl()). polish() must NOT delete an existing pasted value silently — if the old cell holds a URL, the replacement label row includes "old value preserved below, copy it to Info!" and the value stays put one row down.`setup()` asserts Scores headers (`team` present — not legacy `player` — and `h1..h18` all present), logging loudly and throwing on mismatch.
