...
### Task 1c (record — executed per spec §20 amendment 2; see ledger)
Suppression label honesty: board par-relative column presents as Total when `courseMap()` is null (`#lbToParHead` flip + `.lb-suppressed` CSS collapse of the duplicate column), glance drops rank claims (keeps thru-N + pending). X37/X38, suite 205→207. Post-review addendum: media-scoped grid-template redefinition for the wide-width suppressed board (whole-branch review Important-1).
