...
1. **Create the form** with these four questions, in order:
   - **Team** (dropdown, menu items hand-filled at draft night — one item per
     team captain; copy the FORM TEAM LIST block from START HERE, built by
...
- **rejected: …** — other validation failures (invalid round, hole out of
  range, invalid score, Info first_tee unreadable, etc.). Resubmit the hole
...
| Run draft night | **Field** tab, `team` column — filling a player's team cell IS drafting them; the site's Draft tab follows live; the pool empties as you type. Afterward, re-run `polish()` and copy the FORM TEAM LIST block from START HERE into the Form's Team dropdown. |
