...
/**
 * qrcode
 * @param typeNumber 1 to 40, or 0 to auto-detect the smallest type that fits
...
for (var qrMathI = 0; qrMathI < 8; qrMathI += 1) {
  QRMath.EXP_TABLE[qrMathI] = 1 << qrMathI;
}
for (var qrMathI = 8; qrMathI < 256; qrMathI += 1) {
  QRMath.EXP_TABLE[qrMathI] = QRMath.EXP_TABLE[qrMathI - 4]
    ^ QRMath.EXP_TABLE[qrMathI - 5]
    ^ QRMath.EXP_TABLE[qrMathI - 6]
    ^ QRMath.EXP_TABLE[qrMathI - 8];
}
for (var qrMathI = 0; qrMathI < 255; qrMathI += 1) {
  QRMath.LOG_TABLE[QRMath.EXP_TABLE[qrMathI] ] = qrMathI;
}
